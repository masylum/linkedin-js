...
module.exports = function (api_key, api_secret, redirect) {
  ...
  // PRIVATE
      ...
      get = function (path, params, token, callback) {
        oAuth.get(rest_base + path, token.oauth_token, token.oauth_token_secret, params, 'text/xml; charset=UTF-8', requestCallback(callback));
      },
  ...
  client.apiCall = function (method, path, params, callback) {
    ...
    if (method === 'GET') {
      get(path, dummyObjToXML(params), token, callback);
    } else if (method === 'POST') {
  ...
  client.getAccessToken = function (req, res, callback) {
    ...
    // Acces token
    if (parsedUrl.query && parsedUrl.query.oauth_token && req.session.auth.linkedin_oauth_token_secret) {
